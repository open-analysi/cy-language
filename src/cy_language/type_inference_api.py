"""Public API for type inference in Cy language.

This module provides the main entry point for external systems (like Backend-Y)
to perform type inference on Cy scripts.
...
def _parse_tool_registry(tool_registry: dict[str, Any]) -> list[ToolSignature]:
    """Parse tool registry dict into ToolSignature objects.

    Args:
        tool_registry: Dict mapping tool names to type signatures
                      Format: {
                          "tool_name": {
                              "parameters": {...},
                              "return_type": {...}
